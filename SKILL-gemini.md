...
Follow these phases in order. Every deterministic helper is available as a Python module under `scripts/`, and every LLM phase dispatches via the `@generalist` sub-agent.

### Setup: resolve paths in EVERY Bash tool call

⚠️ **CRITICAL: Gemini CLI's Bash tool runs every command in a fresh shell. Variables set in one Bash tool call do NOT persist to the next.** If you set `SKILL_ROOT` in one Bash call and reference `"$SKILL_ROOT/..."` in the next, `$SKILL_ROOT` will be empty and the command will fail with `permission denied` or `file not found`.
...
- `$MODEL`: your own model identifier. For Gemini CLI, that's `gemini-2.5-pro` (or gemini-2.5-flash). You know what you are. If you genuinely don't, omit `--model`; null is better than fabricated.
- `$HOST`: which AI CLI you're running in. For Gemini CLI the value is exactly `gemini_cli`. You know this — it's literally the CLI that loaded this SKILL.md. If omitted, the log script falls back to env-var sniffing and then to `"unknown"`, which is what we want to avoid.
...
This skill runs on Gemini CLI; the same SKILL.md is generated per-host from a shared template (Claude Code, Codex CLI, Gemini CLI, and OpenCode are all supported). The contract is identical across hosts: present 2+ real options, let the student type free text in an "Other" field. On Gemini CLI the question tool is `ask_user`.

Wherever this document says "use the question tool" or names `ask_user`, that's the tool to call. Reference it with backticks — models match fenced tool names more reliably than bare prose.

⚠️ **`ask_user` requires a MINIMUM of 2 real options.** "Other" is auto-added and does NOT count toward the minimum. Supplying only 1 option crashes with `InputValidationError: Too small: expected array to have >=2 items`. This is the #1 first-run-setup bug to avoid.
...
If `ask_user` is not available (e.g., `codex exec` non-interactive mode, a CI script run, or a host config that didn't enable it), do NOT guess values from context. Silent inference produced wrong configs for ambiguous inputs in v0.1 — students got run-time decisions they didn't make.
...
- **Dispatch:** the `@generalist` built-in generalist sub-agent.
...
Use the host's question tool (`ask_user`) to collect the remaining values. Follow the "Interactive prompt pattern" section above: every free-text field uses a 2-option question where the student pastes the answer in Other. Do NOT create a three-option "I'll provide it" middleman.
...
The `install_scope_path` field is auto-detected by the log script from the skill's own path ($HOME/.gemini/skills/... → `user_home`; other locations → `project_local`). You don't need to pass it explicitly.

Substitute `$STYLE` with the chosen style ("eu" or "us"), `$PHOTO_INCLUDED` with "true" or "false", `$GITHUB_CONFIGURED` with "true" or "false" depending on whether `github_username` is set. For `$MODEL` substitute `gemini-2.5-pro` (or whichever Gemini CLI model you're actually running). For `$HOST` substitute the literal `gemini_cli`. The script never exits non-zero; its failures are silent so the student never sees telemetry errors.
...
If `mode == "url"`: fetch the page with the `web_fetch` tool. If the returned text is shorter than 500 characters or clearly a login wall (contains "Sign in", "Log in", or similar without the JD content), prompt the student via the host's question tool (`ask_user`) to paste the JD text manually, then treat the response as `mode: "literal"` (no `--url` needed in the format-jd call since the student's paste has no URL).
...
Use the host's question tool (`ask_user`) with:
...
**Avoid** `FOLDER_SUMMARY='...'; echo "$FOLDER_SUMMARY" > file` — that's the broken pattern; use the heredoc form above. (Gemini CLI doesn't ship a shell-free Write tool, so heredoc is your only safe option.)
...
If `COMPANY` is empty (fit-analyst returned `UNKNOWN` or no line): prompt the student once via the host's question tool (`ask_user`): "I couldn't identify the company from the JD. What company is this role at?" Use the response as `COMPANY`.
...
Dispatch a sub-agent with `$PROMPT` as its instruction text. Unlike the other sub-agents, company-researcher MUST have web-search and web-fetch tools available — those are the whole point of this task. On Gemini CLI: `web_search` and `web_fetch`. The compiled prompt asks for 3-5 recent company facts with parenthetical citations. Template: `scripts/prompts.py` `company-researcher` kind.

If the sub-agent returns a FAILURE sentinel, prompt the student via the host's question tool (`ask_user`): "Company research failed (<reason>). Paste 2-3 bullets of what you already know about {company}, or leave blank to accept a generic cover letter."
...
Dispatch BOTH sub-agents in the same message with two `@generalist` calls. They have no dependency on each other, and running in parallel saves ~30-45 seconds.
...
**Dispatch cover-letter and interview-coach in parallel** — in one orchestrator turn, issue both sub-agent calls with `$PROMPT_COVER` and `$PROMPT_PREP` respectively. On Gemini CLI this is two `@generalist` calls in the same message.
...
3. Batch questions — up to 4 bullets per question-tool call (`ask_user` supports batching 2-4 questions per call on every supported host). For each bullet:
...
**Every call-site below should include both `--host "$HOST"` and `--model "$MODEL"`.** `$HOST` is the AI CLI you're running in; for Gemini CLI that's the literal `gemini_cli`. `$MODEL` is your own model identifier (e.g. `gemini-2.5-pro`). You substitute both as literal strings — you know what CLI you are and what model you are. Example for Gemini CLI:

- `--host gemini_cli --model gemini-2.5-pro` (or gemini-2.5-flash)
...
installation path — user-scope (`$HOME/.gemini/skills/...`) → `user_home`; anywhere
