...
Follow these phases in order. Every deterministic helper is available as a Python module under `scripts/`, and every LLM phase dispatches via the `task` tool with `subagent_type="general"`.

### Setup: resolve paths in EVERY Bash tool call

⚠️ **CRITICAL: OpenCode's Bash tool runs every command in a fresh shell. Variables set in one Bash tool call do NOT persist to the next.** If you set `SKILL_ROOT` in one Bash call and reference `"$SKILL_ROOT/..."` in the next, `$SKILL_ROOT` will be empty and the command will fail with `permission denied` or `file not found`.
...
- `$MODEL`: your own model identifier. For OpenCode, that's `anthropic/claude-opus-4-7` (or or whichever provider/model you're configured against — OpenCode uses the `provider/model` format). You know what you are. If you genuinely don't, omit `--model`; null is better than fabricated.
- `$HOST`: which AI CLI you're running in. For OpenCode the value is exactly `opencode_cli`. You know this — it's literally the CLI that loaded this SKILL.md. If omitted, the log script falls back to env-var sniffing and then to `"unknown"`, which is what we want to avoid.
...
This skill runs on OpenCode; the same SKILL.md is generated per-host from a shared template (Claude Code, Codex CLI, Gemini CLI, and OpenCode are all supported). The contract is identical across hosts: present 2+ real options, let the student type free text in an "Other" field. On OpenCode the question tool is `question`.

Wherever this document says "use the question tool" or names `question`, that's the tool to call. Reference it with backticks — models match fenced tool names more reliably than bare prose.

⚠️ **`question` requires a MINIMUM of 2 real options.** "Other" is auto-added and does NOT count toward the minimum. Supplying only 1 option crashes with `InputValidationError: Too small: expected array to have >=2 items`. This is the #1 first-run-setup bug to avoid.
...
If `question` is not available (e.g., `codex exec` non-interactive mode, a CI script run, or a host config that didn't enable it), do NOT guess values from context. Silent inference produced wrong configs for ambiguous inputs in v0.1 — students got run-time decisions they didn't make.
...
Use the host's question tool (`question`) to collect the remaining values. Follow the "Interactive prompt pattern" section above: every free-text field uses a 2-option question where the student pastes the answer in Other. Do NOT create a three-option "I'll provide it" middleman.
...
The `install_scope_path` field is auto-detected by the log script from the skill's own path ($HOME/.opencode/skills/... → `user_home`; other locations → `project_local`). You don't need to pass it explicitly.

Substitute `$STYLE` with the chosen style ("eu" or "us"), `$PHOTO_INCLUDED` with "true" or "false", `$GITHUB_CONFIGURED` with "true" or "false" depending on whether `github_username` is set. For `$MODEL` substitute `anthropic/claude-opus-4-7` (or whichever OpenCode model you're actually running). For `$HOST` substitute the literal `opencode_cli`. The script never exits non-zero; its failures are silent so the student never sees telemetry errors.
...
If `mode == "url"`: fetch the page with the `webfetch` tool. If the returned text is shorter than 500 characters or clearly a login wall (contains "Sign in", "Log in", or similar without the JD content), prompt the student via the host's question tool (`question`) to paste the JD text manually, then treat the response as `mode: "literal"` (no `--url` needed in the format-jd call since the student's paste has no URL).
...
Use the host's question tool (`question`) with:
...
Equivalent on OpenCode: use the Write tool directly with the sub-agent response as the file body — Write doesn't go through a shell at all, so no quoting hazard. **Avoid** `FOLDER_SUMMARY='...'; echo "$FOLDER_SUMMARY" > file` — that's the broken pattern.
...
If `COMPANY` is empty (fit-analyst returned `UNKNOWN` or no line): prompt the student once via the host's question tool (`question`): "I couldn't identify the company from the JD. What company is this role at?" Use the response as `COMPANY`.
...
Dispatch the company-researcher sub-agent, giving it web-search access.
...
Dispatch a sub-agent with `$PROMPT` as its instruction text. Unlike the other sub-agents, company-researcher MUST have web-search and web-fetch tools available — those are the whole point of this task. On OpenCode: `websearch` and `webfetch` (note: `websearch` requires `OPENCODE_ENABLE_EXA=1` to be set, or that you've configured OpenCode against a provider that ships search natively). The compiled prompt asks for 3-5 recent company facts with parenthetical citations. Template: `scripts/prompts.py` `company-researcher` kind.

If the sub-agent returns a FAILURE sentinel, prompt the student via the host's question tool (`question`): "Company research failed (<reason>). Paste 2-3 bullets of what you already know about {company}, or leave blank to accept a generic cover letter."
...
Dispatch BOTH sub-agents in the same message with two `task` calls. They have no dependency on each other, and running in parallel saves ~30-45 seconds.
...
**Dispatch cover-letter and interview-coach in parallel** — in one orchestrator turn, issue both sub-agent calls with `$PROMPT_COVER` and `$PROMPT_PREP` respectively. On OpenCode this is two `task` calls in the same message (parallel works in current builds but may serialize — see dispatch notes earlier in this doc).
...
3. Batch questions — up to 4 bullets per question-tool call (`question` supports batching 2-4 questions per call on every supported host). For each bullet:
...
**Every call-site below should include both `--host "$HOST"` and `--model "$MODEL"`.** `$HOST` is the AI CLI you're running in; for OpenCode that's the literal `opencode_cli`. `$MODEL` is your own model identifier (e.g. `anthropic/claude-opus-4-7`). You substitute both as literal strings — you know what CLI you are and what model you are. Example for OpenCode:

- `--host opencode_cli --model anthropic/claude-opus-4-7` (or whichever provider/model you're configured against — OpenCode uses the `provider/model` format)

If you genuinely don't know the model ID, omit `--model` (null is better than fabricated). Same rule for `--host`: omit rather than guess. The edge function caps both at 40 chars; no enum validation on model (space moves too fast), but host should match one of `claude_code` / `codex_cli` / `gemini_cli` / `opencode_cli`.
...
installation path — user-scope (`$HOME/.opencode/skills/...`) → `user_home`; anywhere
else → `project_local`. No orchestrator substitution needed. `$SETUP_DURATION_SECONDS`
is time elapsed since the consent prompt started.
