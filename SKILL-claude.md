...
for c in \
  ...
  "$REPO_ROOT/.claude/skills/resumasher"; do
  [ -n "$c" ] || continue
...
- `$MODEL`: your own model identifier. For Claude Code, that's `claude-opus-4-7` (or claude-sonnet-4-6, claude-haiku-4-5). You know what you are. If you genuinely don't, omit `--model`; null is better than fabricated.
- `$HOST`: which AI CLI you're running in. For Claude Code the value is exactly `claude_code`. You know this — it's literally the CLI that loaded this SKILL.md. If omitted, the log script falls back to env-var sniffing and then to `"unknown"`, which is what we want to avoid.
...
This skill runs on Claude Code; the same SKILL.md is generated per-host from a shared template (Claude Code, Codex CLI, Gemini CLI, and OpenCode are all supported). The contract is identical across hosts: present 2+ real options, let the student type free text in an "Other" field. On Claude Code the question tool is `AskUserQuestion`.

Wherever this document says "use the question tool" or names `AskUserQuestion`, that's the tool to call. Reference it with backticks — models match fenced tool names more reliably than bare prose.

⚠️ **`AskUserQuestion` requires a MINIMUM of 2 real options.** "Other" is auto-added and does NOT count toward the minimum. Supplying only 1 option crashes with `InputValidationError: Too small: expected array to have >=2 items`. This is the #1 first-run-setup bug to avoid.
...
If `AskUserQuestion` is not available (e.g., `codex exec` non-interactive mode, a CI script run, or a host config that didn't enable it), do NOT guess values from context. Silent inference produced wrong configs for ambiguous inputs in v0.1 — students got run-time decisions they didn't make.
...
Use the host's question tool (`AskUserQuestion`) to collect the remaining values. Follow the "Interactive prompt pattern" section above: every free-text field uses a 2-option question where the student pastes the answer in Other. Do NOT create a three-option "I'll provide it" middleman.
...
Substitute `$STYLE` with the chosen style ("eu" or "us"), `$PHOTO_INCLUDED` with "true" or "false", `$GITHUB_CONFIGURED` with "true" or "false" depending on whether `github_username` is set. For `$MODEL` substitute `claude-opus-4-7` (or whichever Claude Code model you're actually running). For `$HOST` substitute the literal `claude_code`. The script never exits non-zero; its failures are silent so the student never sees telemetry errors.
...
Use the host's question tool (`AskUserQuestion`) with:
...
Equivalent on Claude Code: use the Write tool directly with the sub-agent response as the file body — Write doesn't go through a shell at all, so no quoting hazard. **Avoid** `FOLDER_SUMMARY='...'; echo "$FOLDER_SUMMARY" > file` — that's the broken pattern.
...
If `COMPANY` is empty (fit-analyst returned `UNKNOWN` or no line): prompt the student once via the host's question tool (`AskUserQuestion`): "I couldn't identify the company from the JD. What company is this role at?" Use the response as `COMPANY`.
...
Dispatch a sub-agent with `$PROMPT` as its instruction text. Unlike the other sub-agents, company-researcher MUST have web-search and web-fetch tools available — those are the whole point of this task. On Claude Code: `WebSearch` and `WebFetch`. The compiled prompt asks for 3-5 recent company facts with parenthetical citations. Template: `scripts/prompts.py` `company-researcher` kind.

If the sub-agent returns a FAILURE sentinel, prompt the student via the host's question tool (`AskUserQuestion`): "Company research failed (<reason>). Paste 2-3 bullets of what you already know about {company}, or leave blank to accept a generic cover letter."
...
**Dispatch cover-letter and interview-coach in parallel** — in one orchestrator turn, issue both sub-agent calls with `$PROMPT_COVER` and `$PROMPT_PREP` respectively. On Claude Code this is two `Task` calls in the same message.
...
3. Batch questions — up to 4 bullets per question-tool call (`AskUserQuestion` supports batching 2-4 questions per call on every supported host). For each bullet:
...
**Every call-site below should include both `--host "$HOST"` and `--model "$MODEL"`.** `$HOST` is the AI CLI you're running in; for Claude Code that's the literal `claude_code`. `$MODEL` is your own model identifier (e.g. `claude-opus-4-7`). You substitute both as literal strings — you know what CLI you are and what model you are. Example for Claude Code:

- `--host claude_code --model claude-opus-4-7` (or claude-sonnet-4-6, claude-haiku-4-5)
...
installation path — user-scope (`$HOME/.claude/skills/...`) → `user_home`; anywhere
