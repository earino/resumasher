...
Follow these phases in order. Every deterministic helper is available as a Python module under `scripts/`, and every LLM phase dispatches via an explicit prose instruction to spawn a sub-agent.

### Setup: resolve paths in EVERY Bash tool call

⚠️ **CRITICAL: Codex CLI's Bash tool runs every command in a fresh shell. Variables set in one Bash tool call do NOT persist to the next.** If you set `SKILL_ROOT` in one Bash call and reference `"$SKILL_ROOT/..."` in the next, `$SKILL_ROOT` will be empty and the command will fail with `permission denied` or `file not found`.
...
- `$MODEL`: your own model identifier. For Codex CLI, that's `gpt-5-codex` (or gpt-5, gpt-5-mini). You know what you are. If you genuinely don't, omit `--model`; null is better than fabricated.
- `$HOST`: which AI CLI you're running in. For Codex CLI the value is exactly `codex_cli`. You know this — it's literally the CLI that loaded this SKILL.md. If omitted, the log script falls back to env-var sniffing and then to `"unknown"`, which is what we want to avoid.
...
This skill runs on Codex CLI; the same SKILL.md is generated per-host from a shared template (Claude Code, Codex CLI, Gemini CLI, and OpenCode are all supported). The contract is identical across hosts: present 2+ real options, let the student type free text in an "Other" field. On Codex CLI the question tool is `request_user_input` (NOT `ask_user_question` — that's an unshipped enhancement request).

Wherever this document says "use the question tool" or names `request_user_input`, that's the tool to call. Reference it with backticks — models match fenced tool names more reliably than bare prose.

⚠️ **`request_user_input` requires a MINIMUM of 2 real options.** "Other" is auto-added and does NOT count toward the minimum. Supplying only 1 option crashes with `InputValidationError: Too small: expected array to have >=2 items` (`"request_user_input requires non-empty options for every question"` on Codex). This is the #1 first-run-setup bug to avoid.
...
If `request_user_input` is not available (e.g., `codex exec` non-interactive mode, a CI script run, or a host config that didn't enable it), do NOT guess values from context. Silent inference produced wrong configs for ambiguous inputs in v0.1 — students got run-time decisions they didn't make.
...
Use the host's question tool (`request_user_input`) to collect the remaining values. Follow the "Interactive prompt pattern" section above: every free-text field uses a 2-option question where the student pastes the answer in Other. Do NOT create a three-option "I'll provide it" middleman.
...
The `install_scope_path` field is auto-detected by the log script from the skill's own path ($HOME/.codex/skills/... → `user_home`; other locations → `project_local`). You don't need to pass it explicitly.

Substitute `$STYLE` with the chosen style ("eu" or "us"), `$PHOTO_INCLUDED` with "true" or "false", `$GITHUB_CONFIGURED` with "true" or "false" depending on whether `github_username` is set. For `$MODEL` substitute `gpt-5-codex` (or whichever Codex CLI model you're actually running). For `$HOST` substitute the literal `codex_cli`. The script never exits non-zero; its failures are silent so the student never sees telemetry errors.
...
If `mode == "url"`: fetch the page with curl via Bash (Codex CLI does not ship a native fetch tool — it conflates fetch with search). If the returned text is shorter than 500 characters or clearly a login wall (contains "Sign in", "Log in", or similar without the JD content), prompt the student via the host's question tool (`request_user_input`) to paste the JD text manually, then treat the response as `mode: "literal"` (no `--url` needed in the format-jd call since the student's paste has no URL).
...
Use the host's question tool (`request_user_input`) with:
...
**Avoid** `FOLDER_SUMMARY='...'; echo "$FOLDER_SUMMARY" > file` — that's the broken pattern; use the heredoc form above. (Codex CLI doesn't ship a shell-free Write tool, so heredoc is your only safe option.)
...
If `COMPANY` is empty (fit-analyst returned `UNKNOWN` or no line): prompt the student once via the host's question tool (`request_user_input`): "I couldn't identify the company from the JD. What company is this role at?" Use the response as `COMPANY`.
...
Dispatch a sub-agent with `$PROMPT` as its instruction text. Unlike the other sub-agents, company-researcher MUST have web-search and web-fetch tools available — those are the whole point of this task. On Codex CLI: `web_search` (opt-in — Codex CLI does not ship a separate fetch tool, just instruct the sub-agent to use search). The compiled prompt asks for 3-5 recent company facts with parenthetical citations. Template: `scripts/prompts.py` `company-researcher` kind.

If the sub-agent returns a FAILURE sentinel, prompt the student via the host's question tool (`request_user_input`): "Company research failed (<reason>). Paste 2-3 bullets of what you already know about {company}, or leave blank to accept a generic cover letter."
...
Dispatch BOTH sub-agents in the same message with two sub-agent-spawn calls. They have no dependency on each other, and running in parallel saves ~30-45 seconds.
...
**Dispatch cover-letter and interview-coach in parallel** — in one orchestrator turn, issue both sub-agent calls with `$PROMPT_COVER` and `$PROMPT_PREP` respectively. On Codex CLI instruct the model to spawn two sub-agents concurrently.
...
3. Batch questions — up to 4 bullets per question-tool call (`request_user_input` supports batching 2-4 questions per call on every supported host). For each bullet:
...
**Every call-site below should include both `--host "$HOST"` and `--model "$MODEL"`.** `$HOST` is the AI CLI you're running in; for Codex CLI that's the literal `codex_cli`. `$MODEL` is your own model identifier (e.g. `gpt-5-codex`). You substitute both as literal strings — you know what CLI you are and what model you are. Example for Codex CLI:

- `--host codex_cli --model gpt-5-codex` (or gpt-5, gpt-5-mini)
...
installation path — user-scope (`$HOME/.codex/skills/...`) → `user_home`; anywhere
